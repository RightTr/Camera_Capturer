...
def reset_to_start():
    ...
    try:
        ...
        # 重置索引到0（第一个点）
        print("[步骤3] 重置状态到第一个关键点...")
        controller.fig8_current_index = 0  # 直接设置为0，跳过初始化匹配
        
        # 删除状态文件，强制重新开始
        if os.path.exists(controller.fig8_state_file):
            os.remove(controller.fig8_state_file)
            print(f"        已清除旧状态文件: {controller.fig8_state_file}")
        ...
        if result and isinstance(result, dict):
            ...
            print("✓ 重置成功！")
            print("=" * 50)
            print(f"当前位置: 关键点 {result['point_no']}/{result['total_points']}")
            print(f"Pan角度: {result['pan']:.1f}°")
            print(f"Tilt角度: {result['tilt']:.1f}°")
            print(f"相位: {result['phase']:.0f}°")
